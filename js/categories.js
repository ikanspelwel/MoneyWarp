...
/**
 * Function to go and get the list of categories
	...
	.done(function(json) {
		...
		if(json.data.length == 0) {
			...
			$('#List > div.mytable').append(
				$('<span>').addClass('float-right').text('add categories here').append(
			...
			jQuery.each(json.data, function (id, data) {
				$('#list').append(
					$('<div>').append(
						...
						$('<div>').text(data.notes),
						$('<div>').html(
							$('<i>').addClass('fa fa-pencil-square-o').on('click', function() { DisplayAddUpdate(data.category_id); })
						)
					)
				);
			}); /** End of jQuery.each */
		...
		/** Now showing the list of categories...	 */
		$('#List').show();
...
function DisplayAddUpdate(category_id) {
	...
	if(category_id === undefined) {
		...
		$('#AddUpdate').dialog( "option", "title", "Add Category" );
		...
		/**
		 *  Existing category, we get the category deatils
		...
		.done(function(json) {
			...
			if(json.data == null) {
				/** If there aren't elements in the return array */
				Attention.show( 'That category couldn\'t be found. Please refresh and try again.' );
				return;
			} else {
				/** If there are elements in the return array */
				...
				/** Set the dialog box title */
				$('#AddUpdate').dialog( "option", "title", "Update Category" );
				
				/** Set the dialog buttons */
				$('#AddUpdate').dialog( "option", "buttons", {
					"Update": function() { DoAddUpdate(category_id) },
					"Cancel": function() { $(this).dialog('close'); }
				});
 ...
 * @returns void
 */
function DoAddUpdate(category_id) {
