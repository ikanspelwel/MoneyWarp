...
/**
 * Function to add or update an category in the
...
function AddUpdate($vars) {
    ...
    if(!$errors->ErrorMsg()) {
        ...
        try {
            ...
            if($vars['category_id']) {
                /** If the category_id has been provided it is an updated */

                /** Prepare Statement */
                $updateItem = $db->get_dbh()->prepare( 'UPDATE `category` SET `name` = :name, `notes` = :notes WHERE `user_id` = :user_id AND `category_id` = :category_id' );
                
                /** Execute Statement */
                $updateItem->execute( array(':user_id' => $_SESSION['user_id'], ':category_id' => $vars['category_id'], ':name' => $vars['name'], ':notes' => $vars['notes']) );
                
            } else {
                /** No category_id means it is an add */

                /** Prepare Statement */
                $addItem = $db->get_dbh()->prepare( 'INSERT INTO `category` (`user_id`, `name`, `notes`) VALUES(:user_id, :name, :notes)' );
                ...
                if($addItem->rowCount() != 1) {
                    /**
                     * No rows were inserted, so that user_id/category_id
...
/**
 * Function to return an category via the provided
 * category_id. It will be returned via json so that 
...
function GetItem($vars) {
    ...
    /** Need to get the category out of the Database */
    try {
        /** Initiate my database class */
        $db = new ikanspelwel\MyDatabase();
        
        /** Prepare Statement */
        $getItem = $db->get_dbh()->prepare( 'SELECT * FROM `category` WHERE `user_id` = :user_id AND `category_id` = :category_id' );
        
        /** Execute Statement */
        $getItem->execute( array(':user_id' => $_SESSION['user_id'], ':category_id' => $vars['category_id']) );
...
/**
 * Function to return all the categories associated with
...
function GetList($vars) {
    ...
    /** Need to get all the categories out of the Database */
    try {
        /** Initiate my database class */
        $db = new ikanspelwel\MyDatabase();
        
        /** Prepare Statement */
        $getList = $db->get_dbh()->prepare( 'SELECT * FROM `category` WHERE `user_id` = :user_id' );
